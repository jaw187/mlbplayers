...
internals.generatePlayerhash = function (rawPlays) {

    var players = {};

    var parse = function (rawPlay) {

        var description = rawPlay.des;
        var actions = description.split('.  ');

        for (var i = 0, il = actions.length; i < il; ++i) {
            var action = actions[i];

            if (action.indexOf('to 3rd') > -1) {
                v
            }
        }
    }
